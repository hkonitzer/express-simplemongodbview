...
var start = function(configuration_, callback) {
  ...
  else if (typeof(configuration.modelDefinitionFile) === 'undefined') throw new ConfigurationException('You have to specify a file with the model definitions');
  ...
  for (i = 0, ix = modelDefinitionFiles.length; i < ix; ++i) {
    var modelDefinitions = require(modelDefinitionFiles[i]);
    // get the models
  ...
  // database setup
  if (typeof(configuration.name) !== 'string') throw new ConfigurationException('You have to specify a database name');  
  var port = configuration.port || '27017';
  var opts = configuration.options || {};
  var host = configuration.host || 'localhost';
  db.connect('mongodb://'+host+':'+port+'/'+configuration.name, opts, function(){  
    log.info('mongodbview - creating database connection to "%s" on host "%s:%s", status: %s', configuration.name, host, port, db.connection.readyState);      
