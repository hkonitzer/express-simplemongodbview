...
var C_dataView = Backbone.View.extend({ 
  ...
  initialize: function(opts_) {       
    ...
    $(this.el).append('<div class="oplog">Last operation:<ul><li>Type: <span class="oplogentry oplog_type"></span></li><li>TS: <span class="oplogentry oplog_ts"></span></li><li>Query: <span class="oplogentry oplog_query"></span></li><li>Error: <span class="oplogentry oplog_error"></span></li></ul></div>');
    var props = this.model.get('properties');    
    var propContainerElem = $('<div>Tree:</div>');
    var propListElem = $('<ul>');
    var propElem = null;
    ...
    propContainerElem.append(propListElem);
    $(this.el).append(propContainerElem);
    this.listenTo(this.model, 'change', this.modelChanged);
  },
  modelChanged : function(data) {    
    if(data.changed.lastlog) {
      $(this.el).find('.oplogentry').text('');
      $(this.el).find('.oplog_type').text(data.changed.lastlog.op);
      var ts = new Date(data.changed.lastlog.timestamp);
      if (isNaN(ts.getTime())) {
        ts = data.changed.lastlog.timestamp;
      }
      $(this.el).find('.oplog_ts').text(ts); 
      if (data.changed.lastlog.error && data.changed.lastlog.error !== null) {
        $(this.el).find('.oplog_error').text(data.changed.lastlog.error); 
      } else $(this.el).find('.oplog_error').text('no'); 
      if (data.changed.lastlog.query && data.changed.lastlog.query !== null) {
        var qry = null;
        if (data.changed.lastlog.query instanceof Object) {
          var qrya = [];
          for (var q in data.changed.lastlog.query) {
            qrya.push(q);
            qrya.push('=');
            qrya.push(data.changed.lastlog.query[q]);
            qrya.push(', ');
          }
          qrya.pop();
          qry = qrya.join('');         
        } else qry = data.changed.lastlog.query;
        $(this.el).find('.oplog_query').text(qry); 
      } else $(this.el).find('.oplog_query').text('n/a'); 
      $(this.el).find('.oplog').show();
    }
  },
...
var C_indexView = Backbone.View.extend({      
  ...
  openWorkbench: function(ev){
    ...
    var colview = window.open('/collection/'+model.get('modelName'), 'Collection View', 'scrollbars=yes,resizable=yes,dependent=no,toolbar=no,status=yes,menubar=no,location=no');
    colview.focus();
