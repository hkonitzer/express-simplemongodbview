...
var C_dataView = Backbone.View.extend({ 
  ...
  render : function(model, data, opts) {    
    ...
    for (var key in paths) {
      csvHeader.push(paths[key]);
    }
    var headerDiv = $('<div>', { 'class' : 'dataheader'});;
    var end = new Date().getTime() -  opts.start;
    headerDiv.html('Query returned '+model.models.length+' entries in '+end+'ms  ('+new Date()+')');
    this.$el.append(headerDiv);
    headerDiv = $('<div>', { 'class' : 'dataheader'});;
    for (var c = 0, cx = csvHeader.length; c < cx; ++c) {
      var span = $('<span>', { 'class' : 'datacell'});
      span.html(csvHeader[c]);
      if (c === cx -1) span.addClass('datacelllast');
      headerDiv.append(span);  
    }
    this.$el.append(headerDiv);
    var elem = this.$el;    
    _.each(model.models, function(el, idx) {      
      var div = $('<div>', { 'class' : 'dataline'});
      for (var c = 0, cx = csvHeader.length; c < cx; ++c) {
        var span = $('<div>', { 'class' : 'datacell', title: csvHeader[c] });
        span.addClass(csvHeader[c]);
        if (c === cx -1) span.addClass('datacelllast');
        div.append(span);  
      }
...
var C_indexView = Backbone.View.extend({      
  ...
  setupModel : function(model) {
    ...
    mainTableBodyTRElem.append(tempTD);
    var btnDiv = $('<div>');
    var btnFetch = $('<button>', { style: 'font-size: 0.8em;', text: 'Fetch'}).button({icons: {primary : 'ui-icon-play'}});
    var btnWatch = $('<button>', { style: 'font-size: 0.8em;', text: 'Watch'}).button({icons: {primary : 'ui-icon-refresh'}});
    var btnClear = $('<button>', { style: 'font-size: 0.8em;', text: 'Clear'}).button({icons: {primary : 'ui-icon-cancel'}});
    var btnRemove = $('<button>', { style: 'font-size: 0.8em;', text: 'Hide'}).button({icons: {primary : 'ui-icon-closethick'}});
    btnFetch.data('modelId', model.cid);
    btnWatch.data('modelId', model.cid);
    btnFetch.on('click', this, this.submitQuery);
    btnWatch.on('click', this, function(ev) {
      var that = ev.data;
      if (btnWatch.button('option', 'label') === 'Stop') {        
        window.clearInterval(model.get('watchRefreshTimer'));
        btnWatch.button('option', 'icons', {primary : 'ui-icon-refresh'});
        btnWatch.button('option', 'label', 'Watch');
        btnFetch.button('enable');
      } else {        
        var queryOptionSet = false;
        var watchData = that.getQueryData(model);
        for (var i in watchData) {
          if (i !== 'sort' && i !== 'fields') {
            queryOptionSet = true;
            break;
          }
        ...
        if (queryOptionSet) {
          btnWatch.button('option', 'icons', {primary : 'ui-icon-cancel'});
          btnWatch.button('option', 'label', 'Stop');
          btnFetch.button('disable');
          var watchRefreshTimer = window.setInterval(function() {            
            that.processQuery(model, watchData);
          }, 2000);
          model.set('watchRefreshTimer', watchRefreshTimer);
        } else alert('Need a filter for watch (auto refresh)!');
      }
    });
    btnClear.on('click', model, this.clearControlElements);    
    btnRemove.on('click', model, this.removeWorkbench);
    btnDiv.append(btnFetch);
    btnDiv.append(btnWatch);
    ...
    //mainElem.draggable({ cancel: '.collectionContent', stack: '.collectionMain', scope: '.collectionMain' });
    //mainElem.resizable({ minWidth: tbl.width(), minHeight: 100 });
    mainElem.on('keyup', 'input.controlElem', this, this.submitQuery);
    // init view for data
  ...
  processQuery: function(model, data_) {    
    var timestamp = new Date().getTime();
    var modelElem = $('#' + model.cid);
    var data = data_ || this.getQueryData(model); 
    modelElem.find('.collectionContent').html('<div>Loading&hellip;</div>');
    model.get('dataView').model.fetch({
      reset: true, 
      start: timestamp,
      data: data,
      success:function(model_, response_, options_) {
        localStorage.setItem(model.get('modelName')+'fetchData', JSON.stringify(options_.data));
      }, 
      error : function(model_, response_, options_) {
        modelElem.find('.collectionContent').html('<div class="ui-state-error ui-corner-all"><div><span class="ui-icon ui-icon-alert" style="float: left; margin-right: .3em;"></span>Error: '+response_.responseText+'</div></div>');          
      } 
    });      
  },
  submitQuery: function(ev) {
    var that = ev.data;
    var model = that.model.get($(ev.currentTarget).data('modelId'));     
    var key = ev.keyCode;    
    if (key === 13 || ev.type === 'click') {      
      that.processQuery(model);      
    } else if (key === 27) {
