...
var createCSVFile = function(modelObj, options, callback) {  
  var transformFunction = function(data) {    
    ...
    for (var d in data) {
      ...
      if (csvHeader[headerName]) {
        ...
        if (dataString.indexOf(csvSeparator) > -1) dataString = '"' + dataString + '"'; // escape csv separator char
    ...
    ret+='\n'; 
    if (progress % 100 === 0) options.socket.emit('exportProgress', progress);    
    return ret;
  }
  
  var fstat = fs.statSync(fileDir); // check for temp  
  var fileName = modelObj.modelName+'_'+options.socket.id+'.csv';
  var csvSeparator = options.seperator || ';';
  var headerLength = 0;
  var progress = 0;
  var csvHeader = {}
  var sort = options.sort || { };
  var qry = modelObj.find({}).sort(sort).lean(true);
  ...
  fileStream.on('close', function() {
    ...
    options.socket.emit('exportDone', fileName);
  ...
  qrystream.on('close', function () { 
    options.socket.emit('exportProgress', progress);    
