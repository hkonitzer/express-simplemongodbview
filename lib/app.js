...
var webSocketForDBLog = require(__dirname+'/websockets.js').socket;
...
var start = function(configuration_, callback) {
  ...
  group.then(function(resultArray){    
    ...
    // additional db operation log socket    
    webSocketForDBLog.init(io, '/log');
  });
}
exports.start = start;


exports.logDBOperation = function(error, modelName, timestamp, operation, query) {
  webSocketForDBLog.logDBOperation(error, modelName, timestamp, operation, query);
}
